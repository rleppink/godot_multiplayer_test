...
func _on_host_button_down():
	...
	LobbyManager.start_hosting(port, _create_config())
...
func _on_lobby_state_changed(new_state):
	...
	%HostButton.disabled = is_multiplayering
	%ConnectButton.disabled = is_multiplayering
	%CancelButton.disabled = !is_multiplayering
	...
	%StartGameButton.disabled = !is_multiplayering && !multiplayer.is_server()
